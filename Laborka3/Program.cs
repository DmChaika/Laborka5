...
namespace Table
{
    ...
    public class OOP
    {
        ...
        public struct Log
        {
            ...
            public string info; //Представляет текст как последовательность кодовых единиц
            public string item;

            public Log(Actions action, DateTime time, string info, string item) //(Возвращает логарифм указанного числа.)
            {
                this.action = action;
                this.time = time;
                this.info = info;
        ...
        public struct Item
        {
            ...
            public Type ItemType; //(общий тип)
            public double price; //(хранит число с плавающей точкой от ±5.0*10-324 до ±1.7*10308 и занимает 8 байта.)
            public int amount;
        ...
        static void ShowTable(List<Item> items)
        {
            ...
            Console.WriteLine(new String('-', 77));
            foreach (Item item in items) //Метод forEach() выполняет указанную функцию один раз для каждого элемента в массиве.
            {
                item.Print();
                Console.WriteLine(new String('-', 77));
            }
        ...
        static void Add(List<Item> items) //Представляет строго типизированный список объектов, к которым можно получить доступ по индексу. Предоставляет методы для поиска, сортировки и управления списками.
        {
            ...
            while (true)
            {
                ...
                string tmp = Console.ReadLine(); //(локальная переменная)
                if (tmp.ToUpper() == "K" || tmp.ToUpper() == "К") //преобразование строчных букв в прописные.
                {
                    ItemType = Type.K;
                    break;
                }
                else if (tmp.ToUpper() == "O" || tmp.ToUpper() == "О") //преобразование строчных букв в прописные.
                {
                    ItemType = Type.O;
                    break;
                }
            ...
            double price = Math.Round(double.Parse(Console.ReadLine()), 2); //возвращает число, округлённое к ближайшему целому.
        ...
        {
            List<Item> AfterRemove = items; //отображает после удаления
        ...
        static void Update(List<Item> items, int index)
        {
            ...
            TmpItem.number = index; //Сообщает отсчитываемый от нуля индекс первого вхождения указанного символа Юникода или строки в этом экземпляре. Метод возвращает -1, если символ или строка не найдены в этом экземпляре.
        ...
        {
            string info = "";
            DateTime NowTime = DateTime.Now; //Добавляет заданное количество дней, месяцев, лет, часов, минут и секунд к объекту DateTime
            TimeSpan SpanTime = DateTime.Now.TimeOfDay; //Представляет временной интервал.
        ...
        {
            LogTimes.Sort(); //Сортирует элементы или часть элементов в List <T>,либо comprasion
            if (LogTimes.Count == 1)
            {
                return LogTimes[0]; //доступ к элементам массива, индексатора или указателя.
            }
            ...
            TimeSpan MaxDiff = LogTimes[1] - LogTimes[0]; //максимальная разница между двумя элементами
        ...
        static void ShowLog(List<Log> CurrentLog, List<TimeSpan> LogTimes)
        {
            while (CurrentLog.Count > 50)
                CurrentLog.Remove(CurrentLog[0]); //Удаляет первое вхождение определенного объекта из List <T>
        ...
        private static void Main()
        {
            ...
            int choice = 0; //Представляет набор альтернатив в ограничениях грамматики распознавания речи.
